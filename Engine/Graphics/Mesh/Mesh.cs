namespace CorrinoEngine.Graphics.Mesh
{
	...
	public class Mesh : IDisposable
	{
		...
		public void Draw(Camera camera, Matrix4 world, float frame)
		{
			...
			{
				if (frame >= TransformAnimation.Length - 1)
				{
		...
		public void Draw(Camera camera, Matrix4 world, float startFrame, float endFrame)
		{
			var model = world;
			...
			if (this.TransformAnimation != null)
			{
				...
				{
					model = this.TransformAnimation[(int)endFrame] * model;
				}
				else
				{
					var lower = (int)startFrame;
					var upper = lower + 1;
					model = (this.TransformAnimation[lower] + (this.TransformAnimation[upper] - this.TransformAnimation[lower]) * (startFrame - lower)) * model;
				}
			}
			else if (this.Transform != null)
			{
			...
			foreach (var child in this.Children)
			{
				child.Draw(camera, model, startFrame, endFrame);
			}
		}
